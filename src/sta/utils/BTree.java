...
public class BTree<Key extends Comparable<Key>, Value extends Term> {
    ...
    private Collection<Value> getWithChildsSearch(Node x, Key key, int ht) {
        ...
        if (ht == 0) {
            for (int j = 0; j < x.m; j++) {
                ...
                if (children[j].key.toString().startsWith(key.toString()) || value.getSoundex().startsWith(key.toString())) {
